      ...
      <section className=\"mx-auto max-w-7xl\">
        ...
        {posts.length === 0 ? (
          <div className=\"flex flex-col items-center justify-center py-40 text-center\">
            ...
            {admin && (
              <Link href=\"/admin/gallery\" className=\"mt-6 inline-flex items-center rounded-full bg-black px-6 py-3 text-sm text-white transition hover:bg-black/80\">
                이미지 업로드하기
              </Link>
            )}
            <Link href=\"/contact\" className=\"mt-4 inline-flex items-center rounded-full border border-black/20 px-6 py-3 text-sm text-black/60 transition hover:bg-black/5\">
              촬영 문의하기
            </Link>
          </div>
        ) : (
              ...
              {posts.map((post) => (
                <Link key={post.id} href={\"\/portfolio\/\" + post.slug} className=\"group block\">
                  <div className=\"relative aspect-[4/5] w-full overflow-hidden rounded-sm bg-[#ece7e2]\">
                    <Image src={post.cover_image as string} alt={post.title} fill sizes=\"(max-width: 768px) 50vw, 25vw\" className=\"object-cover transition duration-700 group-hover:scale-[1.03]\" />
                  </div>
...
'''
with open('/Users/jaydenbrown/jaydenbrown/app/portfolio/page.tsx', 'w') as f:
    f.write(content)
print('portfolio 완료')
"